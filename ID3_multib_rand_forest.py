...
if __name__ == '__main__':
    ...
    for i in range(K):
        ...
        for j in range(trees):
            forest.append(ID3(random_forest.rand_feature(ID3_baseline.features, feature_div, 3), random_forest.rand_data(train, int(len(train) * 0.5)), cut))
        tp = 0
        for j in test:
            predicts = []
            for t in forest:
        ...
        for k in range(3):
            p, r = eval(ID3_baseline.labels[k], test, forest)
